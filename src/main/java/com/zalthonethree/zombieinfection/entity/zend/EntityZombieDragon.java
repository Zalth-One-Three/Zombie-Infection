...
public class EntityZombieDragon extends EntityLiving implements IBossDisplayData, IEntityMultiPart, IMob { // Welp, it was bound to happen eventually.
	...
	public BlockPos healingEndStone;
	public BlockPos[] endStoneLocations;
	
	/*
	 * Dragon will eat end stone if more than 20 health lost.
	 * Egg size 2F, 2F
	 * Dragon size 16F, 8F
	 */
	...
	private void turnToDragon() {
		...
		this.dragonPartTail2 = new EntityDragonPart(this, "tail", 4F, 4F);
		this.dragonPartTail3 = new EntityDragonPart(this, "tail", 4F, 4F);
		this.dragonPartWing1 = new EntityDragonPart(this, "wing", 4F, 4F);
		this.dragonPartWing2 = new EntityDragonPart(this, "wing", 4F, 4F);
		this.dragonPartArray = new EntityDragonPart[] {
				this.dragonPartBody, this.dragonPartHead, this.dragonPartTail1, this.dragonPartTail2, this.dragonPartTail3, this.dragonPartWing1, this.dragonPartWing2
		};
		this.setSize(16F, 8F);
	...
	public EntityZombieDragon(World worldIn) {
		super(worldIn);
		this.setSize(2F, 2F);
		this.setHealth(10F);
		this.isEgg = true;
	...
	private void updateHealingBlock() {
		...
		if (this.rand.nextInt(10) == 0) {
			for (BlockPos bp : this.endStoneLocations) {
				if (bp != null) {
					if (this.worldObj.getBlockState(bp).getBlock() == Blocks.end_stone) {
						this.healingEndStone = bp;
						break;
					} else {
						bp = null;
					}
	...
	@SuppressWarnings({"rawtypes", "unchecked"}) private void setNewTarget() {
	...
	@SuppressWarnings("rawtypes") private void collideWithEntities(List p_70970_1_) {
	...
	@SuppressWarnings("rawtypes") private void attackEntitiesInList(List p_70971_1_) {
	...
	@Override public boolean attackEntityFromPart(EntityDragonPart dragonPart, DamageSource source, float p_70965_3_) {
		...
		if (this.isEgg) {
			...
			if (entity != null) {
				if (entity instanceof EntityEnderZombie) {
					entity.attackEntityFrom(DamageSource.causeMobDamage(this), ((EntityEnderZombie) entity).getMaxHealth());
					this.setHealth(Math.min(this.getMaxHealth(), this.getHealth() + 10));
					if (this.getHealth() >= this.getMaxHealth()) this.turnToDragon();
				}
			}
			return false;
		}
